...
首次归档见 [输出与归档](../guides/archive.md)，已有数据续写见 [恢复与完整性](../guides/recovery.md)。这两页提供完整配置的运行方式和进度核对步骤。
...
`cargo test -p output-file` 覆盖存储契约；`python3 quality/tests/output-file/verify.py` 驱动真实 Core 与插件，覆盖三种目标、恢复和故障场景。测试使用临时目录，不复用示例归档。
