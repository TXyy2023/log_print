    ...
    with tempfile.TemporaryDirectory(prefix='log-print-measure-')as tmp:
        ...
        try:
            ...
            if is_app:
                ...
                raw_stream='bench.transformed' if mode=='transform' else 'bench'
                specs=[{'id':'raw','bin':str(bins/'output-raw'),'reads':[raw_stream],
                        'config':{'streams':[raw_stream],'from':1}},
                ...
                if mode=='transform':
                    specs.append({'id':'transform','bin':str(bins/'output-transform'),'reads':['bench'],
                        'streams':[{'id':'bench.transformed','parents':['bench']}],
                        'save':{'enabled':False},
                        'config':{'streams':['bench'],'output_stream':'bench.transformed','from':1,
                            'input_encoding':'raw','output_encoding':'raw','radix':'none',
                            'split_lines':False,'keep_newline':True,'flush_partial':True,
                            'prefix':'','suffix':'','delete':[],'replace':[]}})
                    extra['transform_workload']='raw passthrough; no line buffering, edits, or persistence'
                ...
                extra['binary_sha256']={name:hashlib.sha256((bins/name).read_bytes()).hexdigest() for name in ['log-print','log-print-core','input-program','output-raw']+(['output-tui']if mode=='plot'else[])+(['output-transform']if mode=='transform'else[])}
            else:
...
def main():
    ...
    parser.add_argument('--output',default=str(ROOT/'quality/artifacts/benchmarks/local'))
    parser.add_argument('--scenario',choices=['log_print_raw','log_print_transform','transform_compare','log_print_save','tee_raw','tee_save','tee_plot','all'],default='all',
                        help='transform_compare runs fresh raw and transform cases; all includes both plus save comparisons')
    ...
    if args.scenario=='all':
        scenarios=['tee_raw','log_print_raw','log_print_transform','tee_save','log_print_save']
    elif args.scenario=='transform_compare':
        scenarios=['log_print_raw','log_print_transform']
    else:
        scenarios=[args.scenario]
    results={scenario:run_case(args,scenario) for scenario in scenarios}
    if 'log_print_raw' in results and 'log_print_transform' in results:
        metric='observer_minus_source_write_before'
        baseline=results['log_print_raw']['latency'][metric]
        transformed=results['log_print_transform']['latency'][metric]
        comparison={'metric':metric,'baseline_scenario':'log_print_raw',
                    'transform_scenario':'log_print_transform',
                    'baseline_ms':baseline,'transform_ms':transformed,
                    'transform_minus_raw_ms':{key:transformed[key]-baseline[key] for key in baseline},
                    'interpretation':'Difference of statistics from two sequential fresh runs, not paired per-record transform execution time; negative differences are retained.'}
        (Path(args.output)/'transform-comparison.json').write_text(json.dumps(comparison,indent=2))
        print(json.dumps({'transform_comparison':comparison}),flush=True)
